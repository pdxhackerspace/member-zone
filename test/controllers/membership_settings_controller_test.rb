...
class MembershipSettingsControllerTest < ActionDispatch::IntegrationTest
  ...
  # The account age cut-off decides who is eligible for the Slack reminder at all, so it
  # stayed here when the reminder's timing moved to the Reminders page.
  test 'update saves the slack signup account age cut-off' do
    patch membership_settings_url, params: {
      membership_setting: membership_setting_params.merge(slack_signup_reminder_max_account_age_months: 9)
    }

    assert_redirected_to membership_settings_url
    assert_equal 9, @membership_setting.reload.slack_signup_reminder_max_account_age_months
  end

  test 'reminder cadence is no longer edited here' do
    get edit_membership_settings_url
